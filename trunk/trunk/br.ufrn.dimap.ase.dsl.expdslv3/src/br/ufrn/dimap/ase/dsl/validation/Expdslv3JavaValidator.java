...
public class Expdslv3JavaValidator extends AbstractExpdslv3JavaValidator {
	...
	public void checkVariables(ExperimentalDesign plan) {	
		...
		if(plan.getType() == DesignType.CDR) {
			...
			for(Factor f: plan.getFactor()) {
				if(f.getName().getName().equalsIgnoreCase(doe.getTreatment().getName())) {
					achou = 1;
				}
			}
			if(achou==0) {
			...
			achou =0;
			for(Factor f: plan.getFactor()) {
				if(f.getName().getName().equalsIgnoreCase(rcbd.getBlockVariable().getName())) {
					achou = 1;
				}
			}
			if(achou==0) {
			...
			achou =0;
			for(Factor f: plan.getFactor()) {
				if(f.getName().getName().equalsIgnoreCase(ls.getCol().getName())) {
					achou = 1;
				}
			}
			if(achou==0) {
			...
			achou =0;
			for(Factor f: plan.getFactor()) {
				if(f.getName().getName().equalsIgnoreCase(ls.getRow().getName())) {
					achou = 1;
				}
			}
			if(achou==0) {
	...
	public void checkDepVariables(ExperimentalDesign design) {
		...
		for (DepVariable depVar : depVars) {
			for (Factor f : design.getFactor()) {
				if(f.getName().getName().equalsIgnoreCase(depVar.getVariable().getName()))
	...
	public void checkFactor(ExperimentalDesign design) {
			for (Factor f : design.getFactor()) {
				for (Parameter p : design.getParameter()) { {
				if(f.getName().getName().equalsIgnoreCase(p.getVariable().getName()))
